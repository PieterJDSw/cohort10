...
def _persist_evaluation_result(
    ...
    session_status = session_question.session.status if session_question.session else None
    if session_status in {"synthesis_requested", "synthesis_queued", "synthesizing", "scored"}:
        session_question.status = "completed"
    elif session_question.status != "completed":
        session_question.status = "evaluated"
...
def evaluate_question(
    ...
    *,
    ...
    publish_dead_letters: bool = True,
) -> dict | None:
    logger.info(
    ...
    with measure_question_evaluation(session_question.question.type):
        if not str(context["submission"]).strip():
            ...
            if llm_status == "failure" and publish_dead_letters:
                publish_dead_letter(
